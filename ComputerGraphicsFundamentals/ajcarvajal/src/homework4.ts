...
class Homework4App {
    ...
    const fsSource = `#version 100
        ...
        void main() {
            ...
            if (uRenderMode == 1) {
                ...
                gl_FragColor = vec4(0.5 * (texel.rgb + Ncolored), 1.0);
  ...
  private loadScenegraph(): void {
    ...
    MazeCreate(this.maze, 5, 5, -1);
    MazeCreate(this.world2D.tileMap, 16, 16, -1);

    this.scenegraph.Load("../ajcarvajal/library/hw4/eye.obj");
  }
  ...
  private update(): void {
    ...
    if (this.keysPressed.get("r") || this.keysPressed.get("R")) {
      ...
      this.gridCounter.reset();
    }
    if (this.keysPressed.get("n") || this.keysPressed.get("N")) {
      this.gridCounter.nextX();
    }
    ...
    this.player2WorldMatrix.Translate(
      dx * 8 * speed * this.dt,
      dy * 8 * speed * this.dt,
      0.0
    );
  ...
  private draw3DGraphics() {
    ...
    const rotX = Matrix4.makeRotation(Math.sin(this.t0) * 30.0, 1.0, 0.0, 0.0);
    const rotY = Matrix4.makeRotation(Math.cos(this.t0) * 30, 0.0, 1.0, 0.0);
    const rotZ = Matrix4.makeRotation(this.t0 * 0.0, 0.0, 0.0, 1.0);
    ...
    const _rotX = Matrix4.makeRotation(Math.cos(this.t0) * 30.0, 1.0, 0.0, 0.0);
    const _rotY = Matrix4.makeRotation(Math.sin(this.t0) * 30, 0.0, 1.0, 0.0);
    const _rotZ = Matrix4.makeRotation(this.t0 * 0.0, 0.0, 0.0, 1.0);
    const _translationMatrix = Matrix4.makeTranslation(1, 1, -6);
    const _rotationMatrix = Matrix4.multiply3(_rotX, _rotY, _rotZ);
    const _modelViewMatrix = Matrix4.multiply(
      _translationMatrix,
      _rotationMatrix
    );

    const _nrotX = Matrix4.makeRotation(
      Math.cos(this.t0 * 2) * 30.0,
      1.0,
      0.0,
      0.0
    );
    const _nrotY = Matrix4.makeRotation(
      Math.sin(this.t0 * 2) * 30,
      0.0,
      1.0,
      0.0
    );
    const _nrotZ = Matrix4.makeRotation(this.t0 * 0.0, 0.0, 0.0, 1.0);
    const _ntranslationMatrix = Matrix4.makeTranslation(-1, -1, -6);
    const _nrotationMatrix = Matrix4.multiply3(_nrotX, _nrotY, _nrotZ);
    const _nmodelViewMatrix = Matrix4.multiply(
      _ntranslationMatrix,
      _nrotationMatrix
    );
    ...
    if (!this.renderConfig) {
      this.renderConfig = this.scenegraph.UseRenderConfig("default");
    }
    if (this.renderConfig) {
      this.renderConfig.Use();
      this.renderConfig.SetMatrix4f("WorldMatrix", modelViewMatrix);
      this.renderConfig.SetMatrix4f("ProjectionMatrix", projectionMatrix);
      this.renderConfig.SetMatrix4f("CameraMatrix", Matrix4.makeIdentity());
      this.renderConfig.SetUniform3f("CameraPosition", Vector3.make(0, 0, 0));
      this.scenegraph.RenderMesh("eye.obj", this.renderConfig);
      this.renderConfig.SetMatrix4f("WorldMatrix", _modelViewMatrix);
      this.scenegraph.RenderMesh("eye.obj", this.renderConfig);
      this.renderConfig.SetMatrix4f("WorldMatrix", _nmodelViewMatrix);
      this.scenegraph.RenderMesh("eye.obj", this.renderConfig);
    } else {
      shape.draw(
        gl,
