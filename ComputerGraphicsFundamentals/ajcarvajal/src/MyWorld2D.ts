...
class MyWorld2D {
    ...
    renderCel(gl: WebGLRenderingContext, which: number, x: number, y: number, w: number, h: number) {
        ...
        shape.draw(gl, this.vertexIndex, this.colorIndex, this.texCoordIndex);
    }

    draw(gl: WebGLRenderingContext, worldMatrixLocation: WebGLUniformLocation, vertexIndex: number, colorIndex: number, texCoordIndex: number, scaleX = 1, scaleY = 1) {
